...
import uk.co.odinconsultants.sssplayground.joins.RunningAverageMain.{Datum, DatumDelimiter, parsingDatum}
...
class TimestampedStreamingSpec extends WordSpec with Matchers with Eventually {
  ...
  "Aggregated stream" should {
    ...
    def dataToString(id2Data: Map[Int, String]): PayloadFn = { id =>
      new ProducerRecord[String, String](topicName, id2Data(id))
    }
    ...
    "be written to HDFS" ignore {
      ...
      StreamingAssert.assert(query, {
        val count = sink.readFromHdfs[Datum](sinkFile, session).count().toInt
